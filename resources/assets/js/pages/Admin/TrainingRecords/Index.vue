<template lang="html">
  <div class="py-8">
    <index-view ref="index" :url="url" title="Registros">
      <div class="flex-1 flex xs:flex-wrap no-underline text-grey-darker items-center p-4" slot="item" slot-scope="item">
        <div class="xs:w-full sm:flex-1 xs:pb-2">
          <p class="text-red font-bold font-dosis">
            {{item.row.person.full_name}} en 
            {{item.row.workshop.name}}
          </p>
        </div>
      ...
      <template slot="item-actions" slot-scope="item">
        <router-link :to="{name: 'admin.training-records.edit', params: { id: item.row.id }}" class="outline-none focus:outline-none p-2 appearance-none flex items-center" v-tooltip.top-center="'Editar'"><svg-icon src="~@svg/icons/regular/edit.svg" class="block fill-current w-4 h-4" /></router-link>
      </template>

      <template slot="actions">
        <router-link :to="{name: 'admin.training-records.create'}" class="btn bg-teal uppercase text-white">Agregar</router-link>
      </template>
