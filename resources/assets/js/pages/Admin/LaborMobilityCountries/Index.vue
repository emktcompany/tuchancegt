<template lang="html">
  <div class="py-8">
    <index-view ref="index" :url="url" title="Movilidad Laboral - Países">
      ...
      <template slot="item-actions" slot-scope="item">
        <router-link :to="{name: 'admin.labor-mobility-countries.edit', params: { id: item.row.id }}" class="outline-none focus:outline-none p-2 appearance-none flex items-center" v-tooltip.top-center="'Editar'"><svg-icon src="~@svg/icons/regular/edit.svg" class="block fill-current w-4 h-4" /></router-link>
      </template>

      <template slot="actions">
        <router-link :to="{name: 'admin.labor-mobility-countries.create'}" class="btn bg-teal uppercase text-white">Agregar</router-link>
      </template>
        ...
        <div class="flex items-end mb-6 -mx-3">
          <div class="px-3 w-full sm:w-1/3 flex-none">
            <form-field label-class="text-xs" name="term" label="Buscar">
              <input placeholder="País" type="text" v-model="index.query.term" class="form-field text-xs">
            </form-field>
          </div>
...
export default {
  data() {
    return {
      url: '/admin/labor-mobility-countries/:id?'
    }
  }
}
</script>
