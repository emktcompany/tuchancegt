<template lang="html">
  <div class="py-8">
    <index-view ref="index" :url="url" title="Dialectos">
      <div class="flex-1 flex xs:flex-wrap no-underline text-grey-darker items-center p-4" slot="item" slot-scope="item">
        <div class="xs:w-full sm:flex-1 xs:pb-2">
          ...
          <p class="text-grey-dark text-sm pt-2">
            {{item.row.code}}
            <span v-if="item.row.ethnicity">&bull; {{item.row.ethnicity.name}}</span>
          </p>
        </div>
      ...
      <template slot="item-actions" slot-scope="item">
        <router-link :to="{name: 'admin.dialects.edit', params: { id: item.row.id }}" class="outline-none focus:outline-none p-2 appearance-none flex items-center" v-tooltip.top-center="'Editar'"><svg-icon src="~@svg/icons/regular/edit.svg" class="block fill-current w-4 h-4" /></router-link>
      </template>

      <template slot="actions">
        <router-link :to="{name: 'admin.dialects.create'}" class="btn bg-teal uppercase text-white">Agregar</router-link>
      </template>
